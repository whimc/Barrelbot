data modify storage whimc:storage stack_entry set value {loop:true, cycle: 0, code_block:true}
...
execute if data storage whimc:storage {stack_entry: {cycles:true}} run data modify storage whimc:storage stack_entry.cycles set value -1b
