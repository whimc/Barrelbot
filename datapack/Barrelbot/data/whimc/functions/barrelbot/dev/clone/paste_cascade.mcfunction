#> whimc:barrelbot/dev/clone/paste_cascade
#   Pastes a cascade clone

scoreboard players operation $temp whimc.barrelbot.puzzle_id = @s whimc.barrelbot.puzzle_id

#Clean up any invalid inputs
...
execute as @e[type=text_display,limit=1,predicate=whimc:barrelbot/match_id,tag=whimc.barrelbot.puzzle_manager] at @s run function whimc:barrelbot/clone/cascade/iterate_x with storage whimc:macro jump