...
with storage whimc:macro Pos:
    ...
    $execute as @e[type=minecraft:item_display,tag=whimc.barrelbot,x=$(min_x),y=$(min_y),z=$(min_z),dx=$(vol_x),dy=$(vol_y),dz=$(vol_z),tag=!whimc.barrelbot.locked] at @s run data modify block ~ ~ ~ Items set value []
